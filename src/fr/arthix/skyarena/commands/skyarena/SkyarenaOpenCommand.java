...
public class SkyarenaOpenCommand extends CommandExecutor {

    private ArenaManager arenaManager;

    public SkyarenaOpenCommand(SkyArena plugin) {
        setBoth();
        setCommand("open");
        setLength(2);
        setPermission("skyarena.admin.open");
        setUsage("/skyarena open <arena/all> (difficulty)");
        setDescription("Ouvre une ou plusieurs arènes.");
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args[1].equalsIgnoreCase("all")) {
            ...
            sender.sendMessage(i != 0 ? ChatUtils.SKYARENA_PREFIX + i + " arène" + (i == 1 ? "" : "s") + " ouverte" + (i == 1 ? "" : "s") + " avec succès !" : ChatUtils.ERROR_PREFIX + "Tout les arènes sont déjà ouvertes !");
        } else {
            ...
            if (arena != null) {
                if (arena.getArenaState() == ArenaState.CLOSE) {
                    ...
                    sender.sendMessage(ChatUtils.ERROR_PREFIX + "Arène déjà ouverte !");
                    return;
                }
            } else {
            ...
            sender.sendMessage(ChatUtils.SKYARENA_PREFIX + "Arène ouverte avec succès !");
        }
    }
